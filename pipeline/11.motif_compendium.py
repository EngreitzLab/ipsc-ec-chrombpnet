...
# Purpose: Build a non-redundant motif compendium from per-day MoDISco H5 files
#          using MotifCompendium (Kundaje lab).
#
# Input:  2-column TSV (-c): day_name <TAB> /path/to/modisco_counts_results.h5
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="2-column TSV: day_name <TAB> modisco_h5_path",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--min-annotation-score", type=float, default=0.7,
    ...
    with open(config_path) as fh:
        for line in fh:
            ...
            if not os.path.exists(h5_path):
                print(f"  [WARN] {day}: H5 not found, skipping: {h5_path}", file=sys.stderr)
                continue
            modisco_dict[day] = h5_path
    return modisco_dict
...
def main():
    ...
    print(f"@ {len(mc.metadata)} motifs loaded across all days")
    ...
    print(f"@ {len(mc.metadata)} motifs → {n_clusters} clusters "
          f"(threshold={args.threshold})")

    # -------------------------------------------------------------------------
    # 5. Export clustered modisco H5 for FiNeMo (step 5.4)
    #    One averaged pattern per cluster; written to modisco_compiled.h5 so
    #    5.4.run_finemo_unified.sh needs no changes.
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 8. Save full metadata TSV (motif names, source day, TF annotations,
    #    cluster IDs)
    # -------------------------------------------------------------------------
