...
Build a non-redundant motif compendium from per-day MoDISco H5 files
...
def load_config(config_path):
    ...
    with open(config_path) as fh:
        for line in fh:
            ...
            if len(parts) != 2:
                print(
                    f"  [WARN] skipping malformed line: {line}", file=sys.stderr
                )
                continue
            day, h5_path = parts
            if not os.path.exists(h5_path):
                print(
                    f"  [WARN] {day}: H5 not found, skipping: {h5_path}",
                    file=sys.stderr,
                )
                continue
...
def main():
    ...
    print(
        f"@ loaded {len(modisco_dict)} MoDISco H5 paths: {list(modisco_dict.keys())}"
    )
    if not modisco_dict:
    ...
    utils_analysis.export_compendium_meme(
        mc_avg, meme_out, name_col="source_cluster"
    )
