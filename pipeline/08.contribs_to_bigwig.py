...
def parse_args():
    ...
    p.add_argument(
        "--h5", required=True, help="Path to average_shaps.counts.h5"
    )
    p.add_argument(
        "--regions",
        required=True,
    ...
    p.add_argument(
        "--chrom-sizes", required=True, help="Chromosome sizes 2-column TSV"
    )
...
def main():
    ...
    print(
        f"  Shape: {projected.shape}  (N={projected.shape[0]}, seqlen={seqlen})"
    )
