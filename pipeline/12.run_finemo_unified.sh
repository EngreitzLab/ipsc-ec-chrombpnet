...
#          day is produced, enabling direct cross-day comparisons.
#
#          One SLURM array job per day.
#
# Input per day:
#   {averaged_dir}/{day}/{day}_average_shaps.counts.h5  – averaged DeepLIFT scores (step 07)
#   modisco_compiled.h5                                 – unified MoDISco patterns (step 11)
#
# Output (inside finemo_unified_dir/{day}_{peak_type}/):
#   hits.bed.gz + hits.bed.gz.tbi     – tabix-indexed hit calls
#   hits.tsv                          – full hit table
#   finemo_report/                    – HTML report
#
# Usage:
#   sbatch 12.run_finemo_unified.sh            # all days
#   sbatch --array=0 12.run_finemo_unified.sh  # day 0 only
#
# Prerequisites: steps 07 and 11 must have completed.
...
if [[ ! -f "${counts_h5}" ]]; then
    echo "ERROR: ${counts_h5} not found. Run 07.average_contrib_scores.sh first." >&2
    exit 1
fi

out_dir="${finemo_unified_dir}/${day}_${peak_type}"
...
echo "[$(date)] [${day}] Extracting regions from averaged scores..."
...
echo "[$(date)] [${day}] Calling hits (unified modisco)..."
...
echo "[$(date)] [${day}] Fi-NeMo (unified) complete."
echo "  Hits: ${hits_file}"
