...
# =============================================================================
# 11.motif_compendium.sh
# Purpose: Build a non-redundant motif compendium from per-day averaged MoDISco
#          H5 files using MotifCompendium (Kundaje lab).
#
# Input:  Per-day fold-averaged MoDISco H5s (step 09)
#           ${averaged_dir}/{day}/modisco/modisco_counts_results.h5
...
#   modisco_config.tsv           – day → H5 path mapping (for reference)
#
# Prerequisites: 09.run_modisco.sh must have completed.
...
# ---------------------------------------------------------------------------
# Build config TSV: day_name <TAB> modisco_h5_path
# Using the fold-averaged MoDISco results (step 09) for all days.
# ---------------------------------------------------------------------------
config_tsv="${modisco_compiled_dir}/modisco_config.tsv"
echo "# day    modisco_h5" > "${config_tsv}"

for day in "${days[@]}"; do
    modisco_h5="${averaged_dir}/${day}/modisco/modisco_counts_results.h5"
    if [[ -f "${modisco_h5}" ]]; then
        echo -e "${day}\t${modisco_h5}" >> "${config_tsv}"
    else
        echo "  [WARN] missing averaged MoDISco H5 for ${day}: ${modisco_h5}" >&2
        echo "  Run 09.run_modisco.sh first." >&2
    fi
done

n_found=$(grep -c "^[^#]" "${config_tsv}" || true)
echo "[$(date)] Config TSV: ${config_tsv} (${n_found} days)"
...
echo ""
echo "  Next step: sbatch 12.run_finemo_unified.sh"
